...
let run (vars, objs) =
  ...
  let rec call odecl actuals globals =
	...
	(Cast.Expr (Call("gtk_label_set_text ", [Call("GTK_LABEL",[Id w]); StrLit s])))
		...
		in let ncenv = 
           ( match fs with
             ...
             | [x] -> [nfdecl] @ [x]
             | x   -> [nfdecl] @ x)
	in (Int 0, (st, gl, ncenv), loc_obj_decls)), env
	...
	in (env, (s, g, ncenv), loc_obj_decls))
  ...
  let env, cenv, _ = (List.fold_left exec ((locals,globals), ([], [], [new_cenv]),  NameMap.empty)
    odecl.obody) in
    (env, cenv)
